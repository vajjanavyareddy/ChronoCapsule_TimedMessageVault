...
# ------------------- SIDEBAR MENU CARDS -------------------
with st.sidebar:
    st.markdown("<h3 style='text-align:center; color:#333;'>📋 Menu</h3>", unsafe_allow_html=True)

    menu_colors = {
        "Create Capsule": "linear-gradient(90deg, #ff7e5f, #feb47b)",
        "View Capsules": "linear-gradient(90deg, #43cea2, #185a9d)",
        "Manage Users": "linear-gradient(90deg, #ff6a00, #ee0979)"
    }

    for m in ["Create Capsule", "View Capsules", "Manage Users"]:
        if st.button(m, key=f"menu_{m}"):
            st.session_state.menu = m
        st.markdown(f"""
            <style>
            div.stButton>button:first-child {{
                background: {menu_colors[m]};
                color: white;
                font-weight: 600;
                font-size: 1.1rem;
                padding: 1rem 0;
